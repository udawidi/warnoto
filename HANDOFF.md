...
- **HARDENING KEAMANAN (asesmen whitebox) — 5 fix kode SELESAI + PUSHED (2026-08-18).** Plan
  ...
  - **OPS TERSISA (RESUME DI SINI — user pergi 2026-08-18):**
    ...
    - **(B) CSP: masih Report-Only, SEDANG diamati (belum enforcing).** Tiap violation resource sah → tambah origin ke `connect-src`/`script-src` di `vercel.json` lalu push. SUDAH ditambah: `wss://warnoto.com`+`wss://*.warnoto.com` (Supabase Realtime, `fc...`), `https://static.cloudflareinsights.com`+`https://cloudflareinsights.com` (CF Insights beacon, `fc94b9f`). **BELUM diamati:** Peta Gudang (Leaflet **tile OSM** — kemungkinan besar butuh `img-src`/`connect-src` `https://*.tile.openstreetmap.org` atau tile server yang dipakai — CEK komponen peta), scan kamera, cari-foto AI, cetak dokumen TUG. **LANGKAH:** user reload `pln.warnoto.com` + F12 Console, pakai semua fitur, kirim tiap `[Report Only] Refused...` → arsitek tambah allowlist → ulang sampai console BERSIH → baru flip header `Content-Security-Policy-Report-Only`→`Content-Security-Policy` di `vercel.json` + push.
    - **(C) LOW:** proksikan `VITE_COHERE/GROQ/OCRSPACE` dari bundle, batas ukuran+sanitasi XLSX import, regen `schema.sql` drift, verifikasi Cloud lama `tadxodrzoquugnsyejld` nonaktif.
...
- 2026-08-21 Claude: **TUG-3/4 Barang Masuk overhaul** — dokumen ke **format resmi PLN** (Bon Penerimaan + BAP pemeriksa Tim Mutu + Lampiran Foto), form disinkron, fuzzy search katalog (Levenshtein≤1), master **Supplier** (tabel `supplier` **applied**), foto barang/lampiran via `PhotoSlot`, **TUG-3/4 PINDAH KE DB** tabel `tug3_transactions` (**applied**, RLS per-UPT, `src/lib/tug3Sync.js`, foto→Storage, blob legacy read-only), **alur baru TL→Asman** (approval Manager/MUP dibuang; TTD BAP tetap Manager), edit-until-TL, draft langsung bernomor, validasi Ajukan (wajib kecuali Amandemen+foto lampiran), history compact, mobile-responsive dicek (tak perlu ubah, pola project sudah ada). 2 migration applied (`20260821_supplier.sql`, `20260821b_tug3_transactions.sql`). Build hijau, unit baseline 4 pre-existing (nol regresi). **Follow-up bug (uji live fajar):** data TUG-3 hilang tiap Ajukan = form TUG-3 tak set `uptId` → upsert kirim `upt_id:null` → NOT NULL violation → 0 baris (cuma nyangkut localStorage). FIXED: `openNewTxn`+`nt3` selalu set `uptId`, guard `tug3Sync` tolak null, toast gagal jadi merah. **E2E RLS tervalidasi** (insert sebagai fajar authenticated UPT-SBY lolos WITH CHECK `can_access_upt`, rollback). Juga fix: status barang per-item (STATUS_SAP: Persediaan/Cadang/Non-SAP → jenisBarang stok), header dok "TUG-3 KARANTINA/PENERIMAAN", TTD "Diserahkan oleh" = PIC+PT. **SISA: user konfirmasi live — buat TUG-3 → row masuk DB → alur TL→Asman → stok masuk → cetak; isolasi cross-UPT.**
