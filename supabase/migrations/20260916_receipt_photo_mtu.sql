-- Proposal only. Apply to self-host after explicit approval.
-- TUG-3 MTU receipt: validate stored item photos and write stock/photo atomically.
...
as $$
declare
  x public.tug3_transactions; r public.mtu_khs_records; a public.profiles; existing public.mtu_khs_receipts;
  item jsonb; items jsonb; item_qty numeric; total_qty numeric := 0; max_qty numeric; stock_row public.stocks;
  stock_id text; location_id text; catalog_id text; catalog_data jsonb; before_qty numeric; after_qty numeric;
  applied_items jsonb := '[]'::jsonb; item_index integer; foto_barang text; foto_path text;
begin
  ...
  for item, item_index in select value, (ordinality - 1)::integer from jsonb_array_elements(items) with ordinality loop
    catalog_id := nullif(btrim(coalesce(item->>'katalogId', '')), '');
    location_id := nullif(btrim(coalesce(item->>'lokasiTujuanId', item->>'lokasiId', '')), '');
    foto_barang := nullif(btrim(coalesce(item->>'fotoBarang', '')), '');
    foto_path := x.id || '/item' || item_index || '-fotoBarang.jpg';
    if foto_barang is null or foto_barang like 'data:%' or foto_barang not in (
         'https://warnoto.com/storage/v1/object/public/tug-photos/' || foto_path,
         'https://api-staging.warnoto.com/storage/v1/object/public/tug-photos/' || foto_path)
       or not exists (select 1 from storage.objects where bucket_id = 'tug-photos' and name = foto_path) then
      raise exception 'MTU_RECEIPT_PHOTO_REQUIRED';
    end if;
  ...
  for item, item_index in select value, (ordinality - 1)::integer from jsonb_array_elements(items) with ordinality loop
    ...
    location_id := nullif(btrim(coalesce(item->>'lokasiTujuanId', item->>'lokasiId', '')), '');
    foto_barang := nullif(btrim(item->>'fotoBarang'), '');
    ...
    if stock_row.id is null then
      ...
      values (stock_id, catalog_id, location_id, r.upt_id, jsonb_build_object(
        ...
        'fotoKeseluruhan', foto_barang, 'img', foto_barang,
      ...
      update public.stocks set upt_id = coalesce(upt_id, r.upt_id), data = jsonb_set(coalesce(data, '{}'::jsonb), '{qty}', to_jsonb(before_qty + item_qty), true)
        || jsonb_build_object('fotoKeseluruhan', foto_barang, 'img', foto_barang, 'uptId', r.upt_id, 'mtuRecordId', r.id, 'mtuTug3Id', x.id, 'source', 'MTU_KHS_TUG3')
    ...
    applied_items := applied_items || jsonb_build_array(jsonb_build_object('stockId', stock_id, 'katalogId', catalog_id,
      'lokasiId', location_id, 'qty', item_qty, 'beforeQty', before_qty, 'afterQty', after_qty, 'itemIndex', item_index, 'fotoBarang', foto_barang));
  end loop;
