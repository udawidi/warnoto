...
-- One transactional boundary for Asman Stock Opname approval.
...
begin
  ...
  -- A lost response may retry the exact same absolute payload safely.
  ...
  -- Catalog is global reference data, but each submitted id must belong to this opname.
  ...
  -- Only absolute rows supplied by the client are written. Every row is tenant-bound.
  ...
  -- Status/data is deliberately last: any validation or upsert error rolls back everything.
