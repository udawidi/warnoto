...
-- Allows ASMAN to save document metadata for finalized Stock Opname documents.
begin;
...
begin
  ...
  if coalesce(v_role, '') not in ('ADMIN','TL','ASMAN','SUPERADMIN') then raise exception 'OPNAME_DOCUMENT_META_ROLE_DENIED'; end if;
