...
import { AkunModal, GantiPasswordModal, ResetPasswordModal } from "./src/components/AkunModals.jsx";
...
export default function PLNWarehouse() {
  ...
  const [maraSatuanOptions, setMaraSatuanOptions] = useState([]); // opsi dropdown "Satuan Default" katalog manual, dari distinct mara_catalog.satuan
  const MARA_SATUAN_FALLBACK = ["BH","U","SET","M","KG","BTG","L","ROL","LBR","PAC","CAN","BOX","TON","PSG","EA"];
  useEffect(() => {
    if (!supabase) return;
    (async () => {
      const { data, error } = await supabase.from("mara_catalog").select("satuan").not("satuan", "is", null).neq("satuan", "");
      const distinct = error ? [] : [...new Set((data||[]).map(r=>r.satuan).filter(Boolean))].sort();
      setMaraSatuanOptions(distinct.length ? distinct : MARA_SATUAN_FALLBACK);
    })();
  }, []);
  ...
  const handleGiMapSaved = row => {
    if (!row?.id) return;
    setGudangList(list => list.map(g => g.__gi && g.giId === row.id ? { ...g, lat: row.lat, lng: row.lng, mapSourceUrl: row.mapSourceUrl } : g));
    setLokasiList(list => list.map(l => l.__gi && l.giId === row.id ? { ...l, lat: row.lat, lng: row.lng, mapSourceUrl: row.mapSourceUrl } : l));
  };
  ...
  const {
    ...
    saveOpname, submitOpname, approveOpname_Asman, approveOpname_Manager, rejectOpname, updateOpnameTugReference, saveOpnameDocumentMeta, deleteOpname,
...
- ${rekomendasi}`);
  ...
  const navItems = (isOperatorRole ? [
    ...
    {id:"transaction",icon:<SidebarIcon name="transaction"/>,label:"TUG"},
    ...(hasRole(currentUser, "TL","ASMAN","MANAGER","ADMIN_UIT","MGR_LOGISTIK_UIT","ADMIN") ? [{id:"approval",icon:<SidebarIcon name="approval"/>,label:"Approval",badge:myPendingApprovals.length + (hasRole(currentUser, "ASMAN")?heavyEquipmentPendingCount:0) + (hasRole(currentUser, "TL","ASMAN") ? gudangCapacityImports.filter(i=>i.status==="PENDING_ASMAN").length : 0) + (hasRole(currentUser, "TL") ? lokasiList.filter(l=>l.status==="PENDING").length : 0) + (hasRole(currentUser, "TL") ? scopedStocks.filter(s=>(s.lokasiMovePending&&["TL", "ASMAN"].includes(s.lokasiMoveApprover))||s.editPending||s.deletePending).length : 0) + (hasRole(currentUser, "ASMAN") ? opnameList.filter(o=>o.status==="PENDING_ASMAN").length : 0) + (hasRole(currentUser, "MANAGER") ? opnameList.filter(o=>o.status==="PENDING_MANAGER").length : 0) + (hasRole(currentUser, "ASMAN") ? stockCountPendingCount : 0)}] : []),
      ...
      <main className="app-main" style={{flex:1,overflowY:"auto",width:isMobile?"100%":"auto",minWidth:0}}>
        ...
        <div style={{display:tab==="opname"?"block":"none"}}>
          <div>
            ...
            <StockOpnameTab
                ...
                saveOpnameDocumentMeta={saveOpnameDocumentMeta}
        ...
        {tab==="stock" && (
          <DataStokTab
            ...
            subGudangList={subGudangList} visibleGudangList={visibleGudangList} stockVisibleGudangList={visibleTugGudangList}
