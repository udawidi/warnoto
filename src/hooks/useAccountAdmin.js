...
export function useAccountAdmin({ currentUser, showToast, reloadUsers }) {
  ...
  function openAddAkun() {
    setAkunForm({username:"", password:"", name:"", role:"VIEWER", jabatan:"", officialPhone:"", notifEvents:[], uptId:"", ultgId:"", uitId:"", pengadaanScope:"UPT", gudangIds:[]});
    setAkunResult(null);
    setAkunModal("add");
  }
  function openEditAkun(u) {
    setAkunForm({id:u.id, username:u.username, password:"", name:u.name||"", role:u.role||"VIEWER", jabatan:u.jabatan||"", officialPhone:u.officialPhone||"", notifEvents:u.notifEvents||[], uptId:u.uptId||"", ultgId:u.ultgId||"", uitId:u.uitId||"", pengadaanScope:u.uitId?"UIT":"UPT", gudangIds:Array.isArray(u.gudangIds)?u.gudangIds:[]});
    setAkunResult(null);
    setAkunModal("edit");
  }
  ...
  async function submitAkunEdit() {
    ...
    const { data, error } = await supabase.functions.invoke("admin-update-user", { body: {
      userId: f.id, name: f.name.trim(), role: f.role, jabatan: f.jabatan||"", officialPhone: f.officialPhone||"", notifEvents: f.notifEvents||[],
  ...
  async function submitAkunBaru() {
    ...
    const { data, error } = await supabase.functions.invoke("admin-create-user", { body: {
      ...
      role: f.role, jabatan: f.jabatan||"", officialPhone: f.officialPhone||"", notifEvents: f.notifEvents||[], uptId: (national || uitScoped) ? "" : (f.uptId||""), ultgId: f.ultgId||"",
