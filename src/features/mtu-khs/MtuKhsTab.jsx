...
import { clearMtuKhsCache, decideMtuKhsChange, decideMtuKhsImport, drainMtuKhsSheetSyncOnce, formatMtuContract, friendlyMtuKhsError, getCachedMtuKhsRecords, loadApprovedTugItems, loadMtuKhsDocuments, loadMtuKhsPendingChanges, loadMtuKhsPendingImports, loadMtuKhsRecords, loadMtuKhsSyncJobs, loadMtuKhsUsage, linkApprovedTugUsage, promoteMtuKhsImport, pushMtuKhsSheetSync, registerMtuKhsDocument, submitMtuKhsChange } from "./mtuKhsApi.js";
...
export function MtuKhsTab({ currentUser, uptList = [], ultgList = [], gudangList = [], supplierList = [], katalogList = [], C, sty, isMobile }) {
  ...
  async function openDetail(record) {
    setSelected(record);
    const [docResult, result, tugResult] = await Promise.all([loadMtuKhsDocuments({ recordId: record.id }), loadMtuKhsUsage(record.id), loadApprovedTugItems()]);
