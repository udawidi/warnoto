...
export function MasterDataTab({ C, sty, currentUser, isMobile, rolePerms, stockSubTab, filteredKatalog, satpamList: rawSatpamList, supplierList, openAddSupplier, openEditSupplier, deleteSupplier, timMutuList: rawTimMutuList, uitList: rawUitList, uptList: rawUptList, ultgList: rawUltgList, users, gudangList: rawGudangList, lokasiList: rawLokasiList, subGudangList, visibleGudangList: rawVisibleGudangList, openAddKatalog, openAddSatpam, openAddUIT, openAddGudang, openAddAkun, importGudangOpen, setImportGudangOpen, showGudangMaintenance, setShowGudangMaintenance, importLokasiOpen, setImportLokasiOpen, gudangCapacityImports, setGudangCapacityImports, saveToCloud, showToast, backfillGudangCoordFromCapacity, dedupeGudangDanSubGudang, isKodeDuplicateInSubGudang, setLokasiList, syncLokasi, maraUploadProgress, maraUploadLoading, uploadMaraToDB, katalogList, katalogSearch, setKatalogSearch, katalogFilterBelumMara, setKatalogFilterBelumMara, pagedKatalog, stocks, openEditKatalog, deleteKatalog, katalogPageSize, setKatalogPageSize, katalogPageClamped, setKatalogPage, katalogTotalPages, openEditSatpam, deleteSatpam, openEditTimMutu, orgSearch, setOrgSearch, collapsedUitIds, setCollapsedUitIds, openAddUPT, openEditUIT, deleteUIT, openAddULTG, openEditUPT, deleteUPT, openEditULTG, deleteULTG, expandedGudangId, setExpandedGudangId, openEditGudang, deleteGudang, showGudangDenahTools, setShowGudangDenahTools, uploadDenahGudang, denahLoading, mapConfigGudangId, setMapConfigGudangId, pendingMapLokasi, setPendingMapLokasi, manualAddMode, setManualAddMode, ocrSuggestGudangId, setOcrSuggestGudangId, ocrSuggestSubGudangId, setOcrSuggestSubGudangId, ocrSuggestions, setOcrSuggestions, assignLokasiKoordinat, suggestKodeFromOcr, expandedSubGudangToolsIds, setExpandedSubGudangToolsIds, uploadDenahSubGudang, denahSubLoading, mapConfigSubGudangId, setMapConfigSubGudangId, pendingMapLokasiSub, setPendingMapLokasiSub, manualAddModeSub, setManualAddModeSub, assignLokasiKoordinatSub, openEditLokasi, requestDeleteLokasi, selectedSubGudangId, setSelectedSubGudangId, openEditAkun, resetMfa, openResetPassword, txns, migratedTug15History, setMigratedTug15History, migrasiPendingReview, setMigrasiPendingReview, maraReference, setMaraReference, setStocks, setKatalogList, setTxns, reloadRolePerms, onGiSaved }) {
  ...
  // Guard: pseudo-gudang/blok GI (`__gi:true`) tak boleh muncul di editor Master Gudang/Lokasi.
  const gudangList = (dataScope === null ? rawGudangList : rawGudangList.filter(g => inScopeUpt(g.uptId, dataScope))).filter(g => !g.__gi);
  const visibleGudangList = (dataScope === null ? rawVisibleGudangList : rawVisibleGudangList.filter(g => inScopeUpt(g.uptId, dataScope))).filter(g => !g.__gi);
  const lokasiList = rawLokasiList.filter(l => !l.__gi);
          ...
          <div className={`workspace-page master-page master-page--${stockSubTab}`}>
            <div className="workspace-page-toolbar">
              <div className="workspace-context-row">
                <span>
                  {stockSubTab==="katalog"?`${filteredKatalog.length} jenis barang terdaftar`:stockSubTab==="satpam"?`${satpamList.length} satpam terdaftar`:stockSubTab==="supplier"?`${supplierList.length} supplier terdaftar`:stockSubTab==="timmutu"?`${timMutuList.length} paket tim mutu`:stockSubTab==="organisasi"?`${uitList.length} UIT • ${uptList.length} UPT • ${ultgList.length} ULTG`:stockSubTab==="garduInduk"?"Gardu Induk dan Bay scoped UPT":stockSubTab==="akun"?`${users.length} akun terdaftar`:stockSubTab==="migrasi"?"Cutover terkontrol data stok dari SAP — wajib backup sebelum apply":`${gudangDisplay.length} gudang • ${lokasiList.length} blok lokasi terdaftar`}
                </span>
              </div>
              <div className="workspace-page-toolbar__actions">
                ...
                {can(currentUser, "aksi.kelolaMaster", rolePerms) && stockSubTab==="supplier" && <button style={sty.btn("primary")} onClick={openAddSupplier}>+ Tambah Supplier</button>}
            ...
            {stockSubTab==="garduInduk" && <MtuKhsMasterPanel currentUser={currentUser} uitList={uitList} uptList={uptList} ultgList={ultgList} onGiSaved={onGiSaved} C={C} />}
            {stockSubTab==="gudang" && (
                ...
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:720}}>
                  <thead>
                    <tr style={{background:C.sidebar,color:"white"}}>
                      {["Foto","No Katalog","Nama Barang","Kategori","Satuan","Status","Aksi"].map(h=>(
                        <th key={h} style={{padding:"8px 10px",textAlign:h==="Aksi"||h==="Foto"?"center":"left",whiteSpace:"nowrap",fontSize:12}}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
            ...
            {/* ── SUB-TAB: SUPPLIER (nasional, tanpa pengelompokan gudang) ── */}
            {stockSubTab==="supplier" && (
              supplierList.length===0 ? <div style={{...sty.card,textAlign:"center",color:C.muted,padding:30}}>Belum ada data Supplier. {can(currentUser, "aksi.kelolaMaster", rolePerms) && "Klik \"+ Tambah Supplier\" untuk menambahkan."}</div> : (
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:14}}>
                  {supplierList.map(sp=>(
                    <div key={sp.id} style={{...sty.card,borderTop:`3px solid ${C.accent}`}}>
                      <div style={{fontWeight:700,fontSize:13,marginBottom:6}}>🚚 {sp.nama}</div>
                      <div style={{fontSize:12,color:C.muted,lineHeight:1.8,marginBottom:10}}>
                        {sp.pic && <div>PIC: {sp.pic}</div>}
                        {sp.telp && <div>Telp: {sp.telp}</div>}
                        {sp.alamat && <div>{sp.alamat}</div>}
                      </div>
                      {can(currentUser, "aksi.kelolaMaster", rolePerms) && (
                        <div style={{display:"flex",gap:6}}>
                          <button style={{...sty.btn("ghost","sm"),flex:1}} onClick={()=>openEditSupplier(sp)}>✏️ Edit</button>
                          <button style={{...sty.btn("danger","sm"),flex:1}} onClick={()=>deleteSupplier(sp.id)}>🗑️ Hapus</button>
                        </div>
                      )}
                    </div>
                  ))}
                    ...
                    <tbody>
                      {filtered.map(({u,tier,unit})=>{
                        ...
                        <tr tabIndex={0} className="mobile-card-table__row" key={u.id} style={{borderBottom:`1px solid ${C.border}`}}>
                          ...
                          <td data-label="Aksi" style={{padding:"8px 6px",display:"flex",gap:6,flexWrap:"wrap"}}>
                            <button style={sty.btn("ghost","sm")} onClick={()=>openEditAkun(u)}>✏️ Edit</button>
                            {hasRole(currentUser,"ADMIN") && <button style={sty.btn("ghost","sm")} onClick={()=>resetMfa(u)}>🔓 Reset 2FA</button>}
                            {hasRole(currentUser,"ADMIN") && <button style={sty.btn("ghost","sm")} onClick={()=>openResetPassword(u)}>🔑 Reset Password</button>}
                          </td>
                        </tr>
                        );
                      })}
