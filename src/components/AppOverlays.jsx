...
export function AppOverlays({ C, isMobile, toast, savingInfo, scannerOpen, handleScanResult, setScannerOpen }) {
  return (
    <>
      {toast && (
        <div style={isMobile
          ? { position: "fixed", top: 16, left: 16, right: 16, zIndex: 9999, background: toast.type === "error" ? C.red : C.green, color: "white", padding: "12px 16px", borderRadius: 10, fontSize: 13, fontWeight: 600, boxShadow: "0 8px 24px rgba(0,0,0,0.25)", textAlign: "center" }
      ...
      {savingInfo && (
        <div style={{ position: "fixed", inset: 0, zIndex: 3000, background: "rgba(15,23,42,0.55)", backdropFilter: "blur(2px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: C.surface, borderRadius: 14, padding: "28px 32px", width: 360, maxWidth: "100%", textAlign: "center", boxShadow: "0 24px 64px rgba(2,6,23,0.35)", borderTop: `4px solid ${C.accent}` }}>
            ...
            <div style={{ fontSize: 13, fontWeight: 800, color: C.text, marginBottom: 4 }}>Menyimpan Transaksi</div>
            ...
            {savingInfo.total > 0 && <div style={{ height: 6, background: "#e2e8f0", borderRadius: 999, overflow: "hidden" }}><div style={{ height: "100%", width: `${Math.round((savingInfo.done / savingInfo.total) * 100)}%`, background: C.accent, borderRadius: 999 }} /></div>}
