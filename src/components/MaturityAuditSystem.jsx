...
import { uploadFileToDrive } from "../lib/gdrive.js";
import { hasRole as defaultHasRole } from "../lib/roles.js";

function defaultCalculateItemLevel(uploadedCount, totalRequired) {
  if (!totalRequired || totalRequired === 0) return 5;
...
// =========================================================================
// COMPONENT 1: DashboardMaturityBanner
// =========================================================================
...
// =========================================================================
// COMPONENT 2: MaturityAuditEditor
// =========================================================================
...
export function MaturityAuditEditor({
  ...
  currentUser,
  hasRole: propsHasRole,
  C = {},
  sty = {},
  isMobile,
  maturityAuditForm = { aspekScores: {}, catatanUPT: "", catatanUIT: "", catatanPusat: "", fileUrl: "", fileNama: "" },
  setMaturityAuditForm,
  maturityAuditEvidence = {},
  setMaturityAuditEvidence,
  expandedAspek: propsExpandedAspek,
  setExpandedAspek: propsSetExpandedAspek,
  activeAspectId: propsActiveAspectId,
  setActiveAspectId: propsSetActiveAspectId,
  aspectPage: propsAspectPage,
  setAspectPage: propsSetAspectPage,
  saveMaturityAudit,
  deleteMaturityAudit,
  maturityAuditSaving,
  calculateItemLevel: propsCalculateItemLevel,
  selectedUpt
}) {
  const checkRole = propsHasRole || defaultHasRole;
  const calculateItemLevel = propsCalculateItemLevel || defaultCalculateItemLevel;
  const safeForm = maturityAuditForm || { aspekScores: {} };
  const safeScores = safeForm.aspekScores || {};
  const safeEvidence = maturityAuditEvidence || {};

  const [internalExpandedAspek, setInternalExpandedAspek] = useState(AUDIT_CATEGORIES[0]?.id || "tata_kelola");
  const [internalAspectPage, setInternalAspectPage] = useState(1);
  const [internalActiveAspectId, setInternalActiveAspectId] = useState(null);
  const [uploadingItems, setUploadingItems] = useState({});

  const expandedAspek = propsExpandedAspek ?? internalExpandedAspek;
  const setExpandedAspek = (id) => {
    setInternalExpandedAspek(id);
    if (propsSetExpandedAspek) propsSetExpandedAspek(id);
  };

  const aspectPage = propsAspectPage ?? internalAspectPage;
  const setAspectPage = (pg) => {
    const val = typeof pg === "function" ? pg(aspectPage) : pg;
    setInternalAspectPage(val);
    if (propsSetAspectPage) propsSetAspectPage(val);
  };
  ...
  const isEdit = maturityAuditModal && maturityAuditModal !== "new";
  const audit = isEdit && typeof maturityAuditModal === "object" ? maturityAuditModal : {};
  const currentUptName = selectedUpt || audit.upt || "UPT Surabaya";
  const isUPT = checkRole(currentUser, "ADMIN", "TL", "ASMAN", "MANAGER", "SUPERADMIN");
  const isUIT = checkRole(currentUser, "ADMIN_UIT", "MGR_LOGISTIK_UIT", "SUPERADMIN");
  const isPusat = checkRole(currentUser, "SUPERADMIN", "MANAGER");
  ...
  const getScore = (item, roleType) => {
    if (roleType === "pusat") {
      const pScore = safeScores[item.id]?.pusat;
      if (pScore > 0) return pScore;
    }
    if (roleType === "uit" || roleType === "pusat") {
      const uScore = safeScores[item.id]?.uit;
      if (uScore > 0) return uScore;
    }
    const uptScore = safeScores[item.id]?.upt;
    if (uptScore > 0) return uptScore;
    const uploadedCount = (safeEvidence[item.id] || []).length;
    return calculateItemLevel(uploadedCount, item.requiredEvidence?.length || 0);
  };
                  ...
                  <div style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 12, flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11, color: "#dbeafe", marginTop: 2 }}>Pilih file foto, PDF, atau dokumen pendukung dari komputer. File akan tersimpan otomatis sesuai sub-bagian.</div>
                      </div>
                    </div>
                    <a
                      href={(() => {
                        const activeAspectFiles = maturityAuditEvidence[activeAspect.id] || [];
                        const firstDriveFile = activeAspectFiles.find(f => f.isDrive && f.driveRepositoryUrl);
                        return firstDriveFile ? firstDriveFile.driveRepositoryUrl : "https://drive.google.com/drive/folders/1PdFTH0qA79v3uS7Y8WkQOkRKiNV2TyxC";
                      })()}
                      target="_blank"
                  ...
                  {activeAspect.requiredEvidence.map((eviItem, eviIdx) => {
                    ...
                    return (
                      <div key={eviItem.id} style={{
                        background: isAutoFilled ? "#f0fdf4" : C.cardBg,
                                ...
                                onChange={async (e) => {
                                  ...
                                  try {
                                    ...
                                    const newFiles = uploadedFiles.map(res => ({
                                      ...
                                      driveRepositoryUrl: res.targetFolderId ? `https://drive.google.com/drive/folders/${res.targetFolderId}` : "https://drive.google.com/drive/folders/1PdFTH0qA79v3uS7Y8WkQOkRKiNV2TyxC",
                                      name: res.name,
                                      size: res.size,
                                      url: res.url,
                                      isDrive: res.isDrive,
                                ...
                                return (
                                  <div key={fi} style={{
                                    ...
                                    <a
                                      ...
                                      title={`File: ${f.name}\n📍 Target Drive Folder: ${fullFolderPath}\n🔗 Repository URL: ${f.driveRepositoryUrl || 'https://drive.google.com/drive/folders/1PdFTH0qA79v3uS7Y8WkQOkRKiNV2TyxC'}`}
                                    >
                                      {f.name}
                                    </a>
                                    <span style={{
              ...
              <div className="matlev-card" style={{ height: "fit-content" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  {AUDIT_CATEGORIES.map(cat => {
                    ...
                    return (
                      <div key={cat.id}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 11, color: C.text || "#0f172a", marginBottom: 4, fontWeight: 600, lineHeight: 1.4 }}>
                          <span>{cat.label}</span>
...
export function Form5STab({ C = {}, sty = {}, currentUser, lokasiList = [], setMaturityAuditEvidence, onBack, isMobile, selectedUpt }) {
  ...
  const addPhotos = async (files) => {
    ...
    try {
      ...
      console.error("Error upload foto 5S ke Google Drive:", err);
      // Fallback lokal jika ada error
